    	...
    	if(linestatus==1){
	    	  try {
				new SettingTask().execute(new String[]{"getallstatus",""});
				 ...
				 Toast.makeText(SettingActivity.this, "访问服务器数据失败，可能是网络忙或者服务器正在维护！", Toast.LENGTH_LONG).show();
				e.printStackTrace();
			}
    	}
    	else{
	 ...
	 class SettingTask extends AsyncTask<String, String, String> {  
			...
			protected void onPostExecute(String result) 
	        {
				...
				if(GetLastErrorStatus()){
					...
					Toast.makeText(SettingActivity.this, httpCommentTools.GetEcption(result), Toast.LENGTH_LONG).show();
				}
	        }  
