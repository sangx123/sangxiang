	        ...
	        builder.setTitle("请选择归属省份") // 标题
	                ...
	                .setItems(DataResours.allValues, new DialogInterface.OnClickListener() {
	                    ...
	                    public void onClick(DialogInterface dialog, int which) {
	                    	if(!TextUtils.isEmpty(DataResours.daima[which])&&!TextUtils.isEmpty(DataResours.sendnum[which])){
	                    		if(choose==0){
		                    		 int simState=((TelephonyManager) getSystemService(BaseActivity.this.TELEPHONY_SERVICE)).getSimState(); 
				            		   if(simState!= TelephonyManager.SIM_STATE_ABSENT){
				            			   FindAndSendMyDialog("发送短信QHMM到"+DataResours.sendnum_pwd[which]+"找回密码", "找回密码","QHMM",DataResours.sendnum_pwd[which]);
				            		  
				            	      }else{
				            	    	  Toast.makeText(BaseActivity.this, "无SIM卡", 3).show();
				            	      }
		                    	}else if(choose==1){
		                    		FindAndSendMyDialog("发送短信"+DataResours.daima[which]+"到"+DataResours.sendnum[which]+"开通免打扰业务", "开通业务",DataResours.daima[which],DataResours.sendnum[which]);
		                    	}else if(choose==2){
		                    		Intent intent=new Intent(BaseActivity.this, YeWuActivity.class);
		                    		startActivity(intent);
		                    	}
                    		}else{
                    			Toast.makeText(BaseActivity.this, "对不起，您所在的省份尚未开通免打扰业务", 3).show();
                    		}
